...
void DeviceStatusEvent::CheckRet(const int32_t eventType, size_t argc, const int32_t value, std::map<int32_t,
    std::shared_ptr<DeviceStatusEventListener>>::iterator& typeHandler)
{
    napi_handle_scope scope = nullptr;
    napi_open_handle_scope(env_, &scope);
    if (scope == nullptr) {
