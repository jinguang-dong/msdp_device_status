...
bool DeviceStatusEvent::Off(int32_t eventType, napi_value handler)
{
    ...
    for (auto listener : eventMap_[eventType]) {
        ...
        if (equal) {
            ...
            if (status != napi_ok) {
                DEV_HILOGE(JS_NAPI, "Delete failed");
                return false;
            }
    ...
    DEV_HILOGI(JS_NAPI, "%{public}d listeners in the list of %{public}d",
        eventMap_[eventType].size(), eventType);
    if (eventMap_[eventType].size() == 0) {
        ret = true;
    }
    napi_close_handle_scope(env_, scope);
    return ret;
...
bool DeviceStatusEvent::OffOnce(int32_t eventType, napi_value handler)
{
    ...
    DEV_HILOGI(JS_NAPI, "%{public}d listeners in the once list of %{public}d",
        eventOnceMap_[eventType].size(), eventType);
    if (eventOnceMap_[eventType].size() == 0) {
        ret = true;
    }
    napi_close_handle_scope(env_, scope);
    return ret;
...
void DeviceStatusEvent::SendRet(int32_t eventType, int32_t value, napi_value &result)
{
    ...
    if (status != napi_ok) {
        DEV_HILOGE(JS_NAPI, "Failed to set_name");
        return;
    }
...
void DeviceStatusEvent::OnEvent(int32_t eventType, size_t argc, int32_t value, bool isOnce)
{
    ...
    DEV_HILOGI(JS_NAPI, "%{public}d callbacks of eventType %{public}d are sent", typeHandler->second.size(), eventType);
...
void DeviceStatusEvent::ClearEventMap()
{
    for (auto iter : eventMap_) {
        for (auto eventListener : iter.second) {
            ...
            if (status != napi_ok) {
                DEV_HILOGE(JS_NAPI, "Failed to delete reference");
                return;
            }
        }
    }
