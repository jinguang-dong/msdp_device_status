...
std::map<int32_t, sptr<IRemoteDevStaCallbck>> DeviceStatusNapi::callbackMap_;
...
napi_value DeviceStatusNapi::SubscribeDeviceStatus(napi_env env, napi_callback_info info)
{
    ...
    if (callbackIter != callbackMap_.end()) {
        ...
        g_DeviceStatusClient.SubscribeCallback(Type(type),
            ActivityEvent(event), ReportLatencyNs(latency), callback);
        callbackMap_.insert(std::pair<int32_t, sptr<IRemoteDevStaCallbck>>(type, callback));
        if (callback == nullptr) {
            return nullptr;
        }
    }
...
napi_value DeviceStatusNapi::UnSubscribeDeviceStatus(napi_env env, napi_callback_info info)
{
    ...
    DEV_HILOGD(JS_NAPI, "UNtype: %{public}d, UNevent: %{public}d", type, event);
    ...
    if (callbackIter != callbackMap_.end()) {
        ...
        g_DeviceStatusClient.UnSubscribeCallback(Type(type), ActivityEvent(event),callbackIter->second);
