...
bool DeviceStatusEvent::Off(int32_t eventType, napi_value handler, bool isOnce)
{
    ...
    if (isOnce) {
        for (auto listener : eventOnceMap_[eventType]) {
            ...
            if (equal) {
                ...
                napi_delete_reference(env_, listener->onHandlerRef);
...
void DeviceStatusEvent::OnEvent(int32_t eventType, size_t argc, int32_t value, bool isOnce)
{
    ...
    DEV_HILOGE(JS_NAPI, "%{public}d callbacks of eventType %{public}d are sent", typeHandler->second.size(), eventType);
