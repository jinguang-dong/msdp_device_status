...
napi_value DevicestatusNapi::SubscribeDevicestatus(napi_env env, napi_callback_info info)
{
    ...
    if (!DevicestatusNapiError::TypeOf(env, args[ARG_0], napi_number)) {
        THROWERR_API9(env, DEVICESTATUS_PARAMETER_ERROR, "type", "number");
        DEV_HILOGD(JS_NAPI, "The first parameter is not number");
        return result;
    }

    if (!DevicestatusNapiError::TypeOf(env, args[ARG_1], napi_function)) {
        THROWERR_API9(env, DEVICESTATUS_PARAMETER_ERROR, "type", "function");
        DEV_HILOGD(JS_NAPI, "The second parameter is not function");
        return result;
    }
