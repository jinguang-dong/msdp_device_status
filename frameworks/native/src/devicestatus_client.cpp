...
int32_t DeviceStatusClient::SubscribeCallback(Type type, ActivityEvent event, ReportLatencyNs latency)
{
    ...
    if (type > Type::TYPE_INVALID && type <= Type::TYPE_LID_OPEN) {
        devicestatusProxy_->Subscribe(type, event, latency);
    }
    return RET_OK;
}

int32_t DeviceStatusClient::UnsubscribeCallback(Type type, ActivityEvent event)
{
    ...
    devicestatusProxy_->Unsubscribe(type, event);
    DEV_HILOGD(INNERKIT, "Exit");
    return RET_OK;
}

int32_t DeviceStatusClient::CreateDataChannel(sptr<IRemoteDevStaCallback> callback)
{
    DEV_HILOGI(INNERKIT, "enter");
    DEV_RET_IF_NULL_WITH_RET((Connect() != RET_OK) || (callback == nullptr), RET_ERR);
    return devicestatusProxy_->CreateDataChannel(callback);
}

int32_t DeviceStatusClient::DestoryDataChannel(sptr<IRemoteDevStaCallback> callback)
{
    DEV_HILOGI(INNERKIT, "enter");
    DEV_RET_IF_NULL_WITH_RET((Connect() != RET_OK) || (callback == nullptr), RET_ERR);
    return devicestatusProxy_->DestoryDataChannel(callback);
}
