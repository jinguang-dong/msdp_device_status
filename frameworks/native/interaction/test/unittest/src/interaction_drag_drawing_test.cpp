...
HWTEST_F(InteractionDragDrawingTest, InteractionDragDrawingTest_Mouse_DragNum_One, TestSize.Level1)
{
    ...
    ASSERT_EQ(ret, RET_OK);
    std::this_thread::sleep_for(std::chrono::milliseconds(TIME_WAIT_FOR_UPDATE_DRAG_STYLE));
    ret = InteractionManager::GetInstance()->UpdateDragCursorStyle(DragCursorStyle::FORBIDDEN);
    ASSERT_EQ(ret, RET_OK);
    std::this_thread::sleep_for(std::chrono::milliseconds(TIME_WAIT_FOR_UPDATE_DRAG_STYLE));
    ret = InteractionManager::GetInstance()->UpdateDragCursorStyle(DragCursorStyle::MOVE);
    ASSERT_EQ(ret, RET_OK);
    std::this_thread::sleep_for(std::chrono::milliseconds(TIME_WAIT_FOR_UPDATE_DRAG_STYLE));
    ret = InteractionManager::GetInstance()->UpdateDragCursorStyle(DragCursorStyle::DEFAULT);
    ASSERT_EQ(ret, RET_OK);
