...
public:
    ...
    bool InitClient();
    int32_t RegisterCoordinationListener(std::shared_ptr<ICoordinationListener> listener, bool isCheckPermission = false);
    int32_t UnregisterCoordinationListener(std::shared_ptr<ICoordinationListener> listener, bool isCheckPermission = false);
    int32_t PrepareCoordination(std::function<void(std::string, CoordinationMessage)> callback, bool isCheckPermission = false);
    int32_t UnprepareCoordination(std::function<void(std::string, CoordinationMessage)> callback, bool isCheckPermission = false);
