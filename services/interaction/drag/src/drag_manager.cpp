...
{
    CALL_DEBUG_ENTER;
    DragData dragData = DATA_ADAPTER.GetDragData();
...
int32_t DragManager::UpdateDragStyle(DragCursorStyle style)
{
    ...
    DATA_ADAPTER.SetDragStyle(style);
...
{
    CALL_DEBUG_ENTER;
    DragCursorStyle style = DATA_ADAPTER.GetDragStyle();
    ...
    dprintf(fd,
            ...
            GetDragCursorStyle(style).c_str(), DATA_ADAPTER.GetDragWindowVisible() ? "true" : "false");
    DragData dragData = DATA_ADAPTER.GetDragData();
    std::string udKey;
...
int32_t DragManager::InitDataAdapter(const DragData &dragData) const
{
    ...
    DATA_ADAPTER.Init(dragData, pointerStyle);
    return RET_OK;
}
...
{
    DATA_ADAPTER.SetDragWindowVisible(visible);
...
{
    return DATA_ADAPTER.GetShadowOffset(offsetX, offsetY, width, height);
}
