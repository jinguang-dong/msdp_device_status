...
int32_t DragDrawing::Init(const DragData &dragData)
{
    ...
    dragExtHandle = dlopen(DRAG_ANIMATION_EXTENSION_SO_PATH.c_str(), RTLD_LAZY);
    if (dragExtHandle == nullptr) {
    ...
    std::shared_ptr<Rosen::RSCanvasNode> shadowNode, std::shared_ptr<Rosen::RSCanvasNode> dragStyleNode)
{
    ...
    auto animationExtFunc = reinterpret_cast<DragStartExtFunc>(dlsym(dragExtHandle, "OnStartDragExt"));
    ...
    std::shared_ptr<Media::PixelMap> stylePixelMap)
{
    ...
    auto animationExtFunc = reinterpret_cast<DragStyleExtFunc>(dlsym(dragExtHandle, "OnDragStyleExt"));
    if (animationExtFunc == nullptr) {
        FI_HILOGE("Failed to get drag extension func");
        DrawStyle(dragStyleNode, stylePixelMap);
        return;
    ...
    std::shared_ptr<Rosen::RSCanvasNode> dragStyleNode)
{
    ...
    auto animationExtFunc = reinterpret_cast<DragSuccessExtFunc>(dlsym(dragExtHandle, "OnStopDragSuccessExt"));
    if (animationExtFunc == nullptr) {
        FI_HILOGE("Failed to get drag extension func");
        RunAnimation(END_ALPHA, END_SCALE_SUCCESS);
        return;
    ...
    if (!handler_->PostTask(std::bind(animationExtFunc, shadowNode, dragStyleNode))) {
        FI_HILOGE("Send animationExtFunc failed");
        RunAnimation(END_ALPHA, END_SCALE_SUCCESS);
    }
}
    ...
    std::shared_ptr<Rosen::RSNode> rootNode)
{
    ...
    auto animationExtFunc = reinterpret_cast<DragFailExtFunc>(dlsym(dragExtHandle, "OnStopDragFailExt"));
    if (animationExtFunc == nullptr) {
        FI_HILOGE("Failed to get drag extension func");
        RunAnimation(END_ALPHA, END_SCALE_FAIL);
        return;
    ...
    if (!handler_->PostTask(std::bind(animationExtFunc, surfaceNode, rootNode))) {
        FI_HILOGE("Send animationExtFunc failed");
        RunAnimation(END_ALPHA, END_SCALE_FAIL);
    }
}
...
DragDrawing::~DragDrawing() {
    if (dragExtHandle != nullptr) {
        dlclose(dragExtHandle);
        dragExtHandle = nullptr;
    }
}
