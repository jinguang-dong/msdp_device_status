    ...
    namespace Msdp {
        namespace DeviceStatus {
            namespace {
                ...
                LABEL {
                LOG_CORE, MSDP_DOMAIN_ID, "DragDrawing"
            };
            constexpr int32_t
            BASELINE_DENSITY {
            160
...
90 };
constexpr int32_t
FIRST_PIXELMAP_INDEX {
0 };
constexpr int32_t
SECOND_PIXELMAP_INDEX {
1 };
constexpr size_t
TOUCH_NODE_MIN_COUNT {
3 };
constexpr size_t
MOUSE_NODE_MIN_COUNT {
4 };
...
-1 };
constexpr int32_t
MOUSE_DRAG_CURSOR_CIRCLE_STYLE {
41 };
constexpr int32_t
CURSOR_CIRCLE_MIDDLE {
2 };
constexpr int32_t
TWICE_SIZE {
2 };
...
constexpr int32_t
TIME_DRAG_CHANGE_STYLE {
50 };
constexpr int32_t
TIME_DRAG_STYLE {
100 };
constexpr int32_t
TIME_STOP_FAIL_WINDOW {
125 };
constexpr int32_t
TIME_STOP_SUCCESS_WINDOW {
250 };
constexpr int32_t
TIME_STOP_SUCCESS_STYLE {
150 };
constexpr int32_t
TIME_STOP {
0 };
...
void DragDrawing::Draw(int32_t
displayId,
int32_t displayX, int32_t
displayY,
bool isNeedAdjustDisplayXY
)
{
if (displayId < 0) {
...
SetBounds(positionX, positionY, g_drawingInfo
.pixelMap->
...
g_drawingInfo.parentNode->
SetFrame(positionX, positionY, g_drawingInfo
.pixelMap->
...
if (!g_drawingInfo.multiSelectedNodes.

empty() &&

!g_drawingInfo.multiSelectedPixelMaps.
...
void DragDrawing::StartStyleAnimation(float startScale, float endScale, int32_t duration) {
    ...
    auto springCurveStyle = endScale == STYLE_END_SCALE
                            ? Rosen::RSAnimationTimingCurve::CreateCubicCurve(BEZIER_030, BEZIER_000, BEZIER_040,
                                                                              BEZIER_100)
                            : Rosen::RSAnimationTimingCurve::CreateCubicCurve(BEZIER_020, BEZIER_000, BEZIER_060,
                                                                              BEZIER_100);
    CHKPV(drawStyleScaleModifier_);
...
void DragDrawing::InitCanvas(int32_t
width,
int32_t height
)
{
...
CHKPV(pixelMapNode);
pixelMapNode->
SetForegroundColor(TRANSPARENT_COLOR_ARGB);
g_drawingInfo.nodes.
emplace_back(pixelMapNode);
std::shared_ptr <Rosen::RSCanvasNode> dragStyleNode = Rosen::RSCanvasNode::Create();
CHKPV(dragStyleNode);
g_drawingInfo.nodes.
emplace_back(dragStyleNode);
if (g_drawingInfo.parentNode == nullptr) {
g_drawingInfo.
parentNode = Rosen::RSCanvasNode::Create();
CHKPV(g_drawingInfo
.parentNode);
}
g_drawingInfo.parentNode->
AddChild(filterNode);
g_drawingInfo.parentNode->
AddChild(pixelMapNode);
if (!g_drawingInfo.multiSelectedPixelMaps.
...
for (
size_t i = 0;
i<multiSelectedNodesSize;
++i) {
g_drawingInfo.rootNode->
AddChild(g_drawingInfo
.multiSelectedNodes[i]);
}
}
}
g_drawingInfo.rootNode->
...
CHKPV(mouseIconNode);
g_drawingInfo.nodes.
emplace_back(mouseIconNode);
g_drawingInfo.rootNode->
AddChild(mouseIconNode);
rsUiDirector_->
SetRoot(g_drawingInfo
.rootNode->
...
int32_t DragDrawing::UpdateSvgNodeInfo(xmlNodePtr curNode, int32_t extendSvgWidth) {
    ...
    oStrStream << xmlGetProp(curNode, BAD_CAST
    "width");
...
xmlNodePtr DragDrawing::UpdateRectNode(int32_t
extendSvgWidth,
xmlNodePtr curNode
)
{
...
std::ostringstream oStrStream;
oStrStream <<
xmlGetProp(curNode, BAD_CAST
"width");
std::string srcRectWidth = oStrStream.str();
if (!
IsNum(srcRectWidth)
) {
FI_HILOGE("srcRectWidth is not digital, srcRectWidth:%{public}s", srcRectWidth.
...
void DragDrawing::UpdateTspanNode(xmlNodePtr curNode) {
    ...
    while (curNode != nullptr) {
        if (!xmlStrcmp(curNode->name, BAD_CAST "tspan")) {
            xmlNodeSetContent(curNode, BAD_CAST
            std::to_string(g_drawingInfo.currentDragNum).c_str());
        }
        curNode = curNode->next;
    }
}
...
void DragDrawing::SetDecodeOptions(Media::DecodeOptions &decodeOpts) {
    ...
    if ((g_drawingInfo.currentStyle == DragCursorStyle::COPY) && (g_drawingInfo.currentDragNum == DRAG_NUM_ONE)) {
        decodeOpts.desiredSize = {
               ...
               (g_drawingInfo.currentStyle == DragCursorStyle::MOVE) &&
               (g_drawingInfo.currentDragNum == DRAG_NUM_ONE)) {
        decodeOpts.desiredSize = {
                .width = DEVICE_INDEPENDENT_PIXEL * GetScaling(),
                .height = DEVICE_INDEPENDENT_PIXEL * GetScaling()
...
int32_t DragDrawing::SetNodesLocation(int32_t
positionX,
int32_t positionY
)
{
...
void DragDrawing::MultiSelectedAnimation(int32_t
positionX,
int32_t positionY, int32_t
adjustSize)
{
size_t multiSelectedNodesSize = g_drawingInfo.multiSelectedNodes.size();
...
for (
size_t i = 0;
(i<multiSelectedNodesSize) && (i<multiSelectedPixelMapsSize); ++i) {
...
[&]() {
multiSelectedNode->
SetBounds(positionX, positionY
+ adjustSize, multiSelectedPixelMap->

GetWidth(),
        multiSelectedPixelMap
