...
public:
    ...
    void UnRegisterStateChange();

    void GetStartId(std::string &startDeviceDhid);
    void GetRemoteId(std::string &remoteNetworkId);
    void GetCoordinationState(CoordinationState &coordinationState);//yijingyoule
...
private:
    ...
    int32_t filterId_ { -1 };
    std::map<CooStateChangeType, std::list<std::function<void(CoordinationState, CoordinationState)>>> stateChangedCallbacks_;
};
