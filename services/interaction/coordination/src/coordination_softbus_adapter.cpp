...
int32_t CoordinationSoftbusAdapter::OnSessionOpened(int32_t sessionId, int32_t result)
{
    ...
    if (result != RET_OK) {
        ...
        std::unique_lock<std::mutex> sessionLock(operationMutex_);
        if (sessionDevMap_.find(networkId) != sessionDevMap_.end()) {
            sessionDevMap_.erase(networkId);
        }
    ...
    void* data, uint32_t dataLen)
{
    ...
    int32_t result = SendBytes(sessionDevMap_[networkId], dataPacket, sizeof(DataPacket) + dataLen);
    free(dataPacket);
    if (result != RET_OK) {
