...
std::unordered_map<CoorType, std::pair<std::string , std::string>> CoordinationDFX::serialStr_ = {
...
int32_t CoordinationDFX::WriteInputFunc(const CoorType &coorType, Types... paras)
{
    ...
    eventType = (static_cast<int32_t>(coorType) & 1) ? OHOS::HiviewDFX::HiSysEvent::EventType::FAULT :
    ...
    const std::string &info)
{
    if(type == INIT_SIGN){
        ...
        return WriteInputFunc(CoorType::OPEN_SOFUBUS_RESULT_THREE, "remoteNetworkId", remoteNetworkId.substr(0, SUB_LEN),
            "status", para);
    }
}
...
{
    if (currentSta != CoordinationState::STATE_OUT || coordinationState_.find(currentSta) == coordinationState_.end()) {
        return RET_ERR;
    }
