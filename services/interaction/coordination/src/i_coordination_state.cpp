...
int32_t ICoordinationState::PrepareAndStart(const std::string &remoteNetworkId, int32_t startDeviceId)
{
    ...
    if (NeedPrepare(remoteNetworkId, originNetworkId)) {
        COO_SM->UpdatePreparedDevices(remoteNetworkId, originNetworkId);
        ret = DISTRIBUTED_ADAPTER->PrepareRemoteInput(
...
{
    CALL_DEBUG_ENTER;
    std::pair<std::string, std::string> networkIds = COO_SM->GetPreparedDevices();
    std::vector<std::string> inputDeviceDhids = COO_DEV_MGR->GetCoordinationDhids(startDeviceId);
    if (inputDeviceDhids.empty()) {
        COO_SM->OnStartFinish(false, networkIds.first, startDeviceId);
        return static_cast<int32_t>(CoordinationMessage::COORDINATION_FAIL);
    }
    int32_t ret = DISTRIBUTED_ADAPTER->StartRemoteInput(
    ...
    bool isSuccess, const std::string &remoteNetworkId, int32_t startDeviceId)
{
    ...
    std::function<void()> handleStartFinishFunc =
        std::bind(&CoordinationSM::OnStartFinish, COO_SM, isSuccess, remoteNetworkId, startDeviceId);
    CHKPV(eventHandler_);
...
{
    CALL_DEBUG_ENTER;
    std::pair<std::string, std::string> prepared = COO_SM->GetPreparedDevices();
