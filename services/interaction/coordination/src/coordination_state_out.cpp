...
int32_t CoordinationStateOut::DeactivateCoordination(const std::string &remoteNetworkId)
{
    ...
    int32_t ret = COO_SOFTBUS_ADAPTER->StopRemoteCoordination(tempRemoteNetworkId);
    if (ret != RET_OK) {
...
void CoordinationStateOut::ProcessStop(const std::string& remoteNetworkId)
{
    ...
    if (inputDeviceDhids.empty()) {
        COO_SM->OnStopFinish(false, remoteNetworkId);
    }
    int32_t ret = DISTRIBUTED_ADAPTER->StopRemoteInput(remoteNetworkId, localNetworkId, inputDeviceDhids,
    ...
    const std::pair<std::string, std::string> &networkIds)
{
    ...
    DISTRIBUTED_ADAPTER->StartRemoteInput(networkIds.first, networkIds.second, inputDeviceDhids, [](bool isSuccess) {});
}
} // namespace DeviceStatus
} // namespace Msdp
} // namespace OHOS
