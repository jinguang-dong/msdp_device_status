    ...
    int32_t& serverFd, int32_t& toReturnClientFd, int32_t& tokenType)
{
    ...
    if (RET_OK != SockOptSetProcess(serverFd, toReturnClientFd, tokenType)) {
        return SockOptSetProcess(serverFd, toReturnClientFd, tokenType);
    }
    ...
    if (!AddSession(sess)) {
        ...
        return CloseSock(serverFd, toReturnClientFd);
    }
...
int32_t StreamServer::SockOptSetProcess(int32_t& serverFd, int32_t& toReturnClientFd, int32_t& tokenType)
{
    CALL_DEBUG_ENTER;
...
int32_t StreamServer::CloseSock(int32_t& serverFd, int32_t& toReturnClientFd)
{
    if (close(serverFd) < 0) {
