...
int DevicestatusService::Dump(int fd, const std::vector<std::u16string>& args)
{
    ...
    DataUtils::Type type;
    std::vector<DataUtils::Data> datas;
    for (type = DataUtils::TYPE_STILL;
        type <= DataUtils::TYPE_LID_OPEN;
        type = (DataUtils::Type)(type+1)) {
        DataUtils::Data data = GetCache(type);
        if (data.value != DataUtils::Value::VALUE_INVALID) {
...
void DevicestatusService::Subscribe(const DataUtils::Type& type,
...
void DevicestatusService::UnSubscribe(const DataUtils::Type& type,
    const DataUtils::ActivityEvent& event,
    const sptr<IdevicestatusCallback>& callback)
{
...
DataUtils::Data DevicestatusService::GetCache(const \
    DataUtils::Type& type)
{
    DEV_HILOGI(SERVICE, "Enter");
    if (devicestatusManager_ == nullptr) {
...
void DevicestatusService::ReportSensorSysEvent(const DataUtils::Type& type, bool enable)
{
