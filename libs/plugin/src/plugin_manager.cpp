...
const std::string wantLib = "/system/lib64/platformsdk/libwant.z.so"; // lzc
const std::string sdkLib = "/system/lib64/libdinput_sdk.z.so"; // lzc
const std::string utilsLib = "/system/lib64/libdinput_utils.z.so"; // lzc
const std::string hfwksdkLib = "/system/lib64/libdhfwk_sdk.z.so"; // lzc
#else
...
const std::string wantLib = "/system/lib/platformsdk/libwant.z.so"; // lzc
const std::string sdkLib = "/system/lib/libdinput_sdk.z.so"; // lzc
const std::string utilsLib = "/system/lib/libdinput_utils.z.so"; // lzc
const std::string hfwksdkLib = "/system/lib/libdhfwk_sdk.z.so"; // lzc
...
int32_t PluginManager::LoadCoordination()
{
    CALL_DEBUG_ENTER;
    // lzc
    // auto hfwksdkHandle = dlopen(hfwksdkLib.data(), RTLD_NOW);
    // if (hfwksdkHandle == nullptr) {
    //     FI_HILOGE("Open hfwksdk failed, name:%{public}s, msg:%{public}s", hfwksdkLib.data(), dlerror());
    //     return RET_ERR;
    // }
    // FI_HILOGE("Open hfwksdk");
    // dlclose(hfwksdkHandle);
    // return RET_OK;

    auto utilsHandle = dlopen(utilsLib.data(), RTLD_NOW);
    if (utilsHandle == nullptr) {
        FI_HILOGE("Open utils failed, name:%{public}s, msg:%{public}s", utilsLib.data(), dlerror());
        return RET_ERR;
    }
    FI_HILOGE("Open utils");
    dlclose(utilsHandle);
    return RET_OK;

    // auto sdkHandle = dlopen(sdkLib.data(), RTLD_NOW);
    // if (sdkHandle == nullptr) {
    //     FI_HILOGE("Open sdk failed, name:%{public}s, msg:%{public}s", sdkLib.data(), dlerror());
    //     return RET_ERR;
    // }
    // FI_HILOGE("Open sdk");
    // dlclose(sdkHandle);
    ...
    // auto wantHandle = dlopen(wantLib.data(), RTLD_NOW);
    // if (wantHandle == nullptr) {
    //     FI_HILOGE("Open want failed, name:%{public}s, msg:%{public}s", wantLib.data(), dlerror());
    //     return RET_ERR;
    // }
    // FI_HILOGE("Open want");
    // dlclose(wantHandle);
...
int32_t PluginManager::UninstallCoordination()
{
    ...
    FI_HILOGE("start Coordination dlclose");
    CHKPR(coordinationHandle_, RET_ERR);
    dlclose(coordinationHandle_);
    FI_HILOGE("Coordination dlclose msg:%{public}s", dlerror());
