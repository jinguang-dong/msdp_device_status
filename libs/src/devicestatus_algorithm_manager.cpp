...
bool AlgoMgr::SensorStart(Type type)
{
    ...
    int32_t sensorTypes = GetSensorTypeId(type);
    if (sensorTypes == 0) {
        DEV_HILOGE(SERVICE, "sensorTypes is empty");
        return false;
    }
        if (CheckSensorTypeId(sensorTypes) == 0) {
            DEV_HILOGE(SERVICE, "Sensor type mismatch");
            return false;
        }
    if (sensorEventCb_ == 0) {
        auto &sensorEventCb_ = SensorDataCallback::GetInstance();
        sensorEventCb_.Init();
    }

    sensorEventCb_->RegisterCallbackSensor(sensorTypes);
    callSensorNum_++;
...
{
    DEV_HILOGD(SERVICE, "enter");
    for (std::vector<Type>::iterator it = algoTypes.begin(); it != algoTypes.end(); it++) {
...
bool AlgoMgr::GetSensorTypeId(Type type)
{
    int32_t sensorTypes;
    if (type == Type::TYPE_STILL || type == Type::TYPE_HORIZONTAL_POSITION
        || type == Type::TYPE_VERTICAL_POSITION) {
        sensorTypes = SensorTypeId::SENSOR_TYPE_ID_ACCELEROMETER;
        return sensorTypes;
    }
    DEV_HILOGD(SERVICE, "GetSensorTypeId failed");
    return false;
}
...
ErrCode AlgoMgr::Disable(Type type)
{
    ...
    UnregisteSensor(type);
    return ERR_OK;
}

ErrCode AlgoMgr::UnregisteSensor(Type type)
{
    DEV_HILOGD(SERVICE, "enter");
    int32_t sensorTypes = GetSensorTypeId(type);
    std::vector<Type> algoTypes;
    algoTypes = GetAlgoType(sensorTypes);
    if (SensorStop(algoTypes)) {
        sensorEventCb_->UnregisterCallbackSensor(sensorTypes);
        callSensorNum_--;
    }
    if (callSensorNum_ == 0) {
