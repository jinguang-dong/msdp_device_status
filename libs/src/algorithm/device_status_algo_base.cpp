...
bool AlgoBase::GetData(int32_t sensorTypeId, AccelData* sensorData)
{
    ...
    if (sensorTypeId != SENSOR_TYPE_ID_ACCELEROMETER) {
        DEV_HILOGE(SERVICE, "sensorTypeId is not Accl");
        return false;
    }
    if (sensorData == nullptr) {
        DEV_HILOGE(SERVICE, "sensorData is null");
        return false;
    }
...
void AlgoBase::Report(OnChangedValue value, int32_t state, Type type)
{
    DEV_HILOGD(SERVICE, "enter");

    if (callback_ == nullptr) {
        DEV_HILOGD(SERVICE, "callback_ is null");
        return;
    }
