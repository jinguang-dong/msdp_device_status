...
void MotionRotate::CalculateDisplayAngle()
{
    ...
    MOTION_HILOGI(MOTION_MODULE_SERVICE, "angle_ : %{public}f", angle_);
    ...
    if (orientation_ > ANGLE_THREE_HUNDRED_AND_SIXTY_DEGREE) {
        ...
        MOTION_HILOGI(MOTION_MODULE_SERVICE, "orientation_bigger_than_360");
    } else if (orientation_ < 0) {
        orientation_ = orientation_ + ANGLE_THREE_HUNDRED_AND_SIXTY_DEGREE;
        MOTION_HILOGI(MOTION_MODULE_SERVICE, "orientation_is_negative");
    ...
    switch (display_) {
        case VERTICAL_90:
            MOTION_HILOGI(MOTION_MODULE_SERVICE, "VERTICAL90_display, display_ : %{public}d", display_);
            Report();
            break;
        case HORIZONTAL_ZERO: {
            MOTION_HILOGI(MOTION_MODULE_SERVICE, "HORIZONTAL_ZERO_display, display_ : %{public}d", display_);
            Report();
            break;
        }
        case VERTICAL_270: {
            MOTION_HILOGI(MOTION_MODULE_SERVICE, "VERTICAL270_display, display_ : %{public}d", display_);
            Report();
            break;
        }
        case HORIZONTAL_180: {
            MOTION_HILOGI(MOTION_MODULE_SERVICE, "HORIZONTAL180_display, display_ : %{public}d", display_);
            Report();
            break;
        }
        default:
...
{
    MOTION_HILOGI(MOTION_MODULE_SERVICE, "%{public}s enter, \
...
void MotionRotate::StartAlgorithm(int32_t sensorTypeId, void* sensorData)
{
    ...
    if ((abs(acc_x_) < ACCELERATION_VALID_THRESHOLD) &&
        ...
        MOTION_HILOGI(MOTION_MODULE_SERVICE, "StartAlgorithm");
        ...
        MOTION_HILOGI(MOTION_MODULE_SERVICE, "ROTATE_pitch_ : %{public}f, ROTATE_roll_ : %{public}f",pitch_, roll_);
        vectorMagnitude_ = sqrt((acc_x_ * acc_x_) + (acc_y_ * acc_y_) + (acc_z_ * acc_z_));
        MOTION_HILOGI(MOTION_MODULE_SERVICE, "ROTATE_vectorMagnitude_ : %{public}f", vectorMagnitude_);
