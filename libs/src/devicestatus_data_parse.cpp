    ...
    Data& data)
{
    ...
    cJSON* cJsonDeviceStatusData = cJSON_GetObjectItem(parser.json_, "data");
    if (cJSON_IsObject(cJsonDeviceStatusData)) {
        cJSON* cJsonType = cJSON_GetObjectItem(cJsonDeviceStatusData, "type");
        cJSON* cJsonValue = cJSON_GetObjectItem(cJsonDeviceStatusData, "value");
        if (cJSON_IsNumber(cJsonType)) {
            data.type = static_cast<Type>(cJsonType->valueint);
        }
        if (cJSON_IsNumber(cJsonValue)) {
            data.value = static_cast<OnChangedValue>(cJsonValue->valueint);
        }
    }

    if (logStatus) {
        DEV_HILOGW(SERVICE, "type:%{public}d, status:%{public}d", data.type, data.value);
    }
    ...
    cJSON* inVector = cJSON_GetObjectItem(parser.json_, DeviceStatusJson[type].json.c_str());
    int32_t inVectorSize = cJSON_GetArraySize(inVector);
    if (inVectorSize == 0) {
        DEV_HILOGE(SERVICE, "InVector size is zero");
        return false;
    }
    inVectorCount_[type] = inVectorCount_[type] % inVectorSize;
    cJSON* cJsonValue = cJSON_GetArrayItem(inVector, inVectorCount_[type]);
    inVectorCount_[type]++;
    if (cJSON_IsNumber(cJsonValue)) {
        return false;
    }
...
bool DeviceStatusDataParse::DisableCount(const Type& type)
{
    ...
    DEV_HILOGI(SERVICE, "before, deviceStatusItem:%{public}d, inVectorCount_:%{public}d",
        deviceStatusItem, inVectorCount_[deviceStatusItem]);
    inVectorCount_[deviceStatusItem] = int(TypeValue::INVALID);
    DEV_HILOGI(SERVICE, "deviceStatusItem:%{public}d, inVectorCount_:%{public}d",
        deviceStatusItem, inVectorCount_[deviceStatusItem]);
    return true;
}
