...
int32_t PointEventCallback::SubscribePointEvent()
{
    ...
    MOTION_HILOGI(MOTION_MODULE_SERVICE, "SubscribePointEvent enter");
...
{
    MOTION_HILOGD(MOTION_MODULE_SERVICE, "OnPointEventChange received pointEvent");
    ...
    MOTION_HILOGI(MOTION_MODULE_SERVICE, "pointAction: %{public}d, desc:%{public}s",
    pointAction, pointEvent->DumpPointerAction());
    MOTION_HILOGI(MOTION_MODULE_SERVICE, "OnPointEventChange pointId:%{public}d coordinate.x: %{public}d, \
    .y: %{public}d",pointId, pointerItem.GetDisplayX(), pointerItem.GetDisplayY());
