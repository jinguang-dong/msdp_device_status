...
int32_t DeviceStatusMsdpMock::TriggerDatabaseObserver()
{
    ...
    if (store_ == nullptr) {
        ...
        InitRdbStore();
        DEV_HILOGE(SERVICE, "TriggerDatabaseObserver failed, error num =%{public}d", errno);
        return ERR_NG;
    }
...
void DeviceStatusMsdpMock::LoopingThreadEntry()
{
    ...
    while (scFlag) {
        ...
        GetEpollEvents(events[cbct], nevents);
    }
}

void DeviceStatusMsdpMock::GetEpollEvents(epoll_event events[], int32_t nevents)
{
