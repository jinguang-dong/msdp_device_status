...
ErrCode DeviceStatusSensorMock::RegisterCallback(const std::shared_ptr<DevicestatusSensorHdiCallback>& callback)
{
...
ErrCode DeviceStatusSensorMock::Enable(DevicestatusDataUtils::DevicestatusType type)
{
    DEV_HILOGD(SERVICE, "Enter");
    int32_t item = int32_t(type);
    enabledType_[item] = int32_t(DevicestatusDataUtils::DevicestatusValue::VALUE_INVALID);
    Init();
    DEV_HILOGD(SERVICE, "Exit");
    return ERR_OK;
}

ErrCode DeviceStatusSensorMock::Disable(DevicestatusDataUtils::DevicestatusType type)
{
    DEV_HILOGD(SERVICE, "Enter");
    scFlag_ = false;
    CloseTimer();
...
ErrCode DeviceStatusSensorMock::DisableCount(const DevicestatusDataUtils::DevicestatusType& type)
{
    DEV_HILOGD(SERVICE, "Enter");
    enabledType_[type] = int32_t(DevicestatusDataUtils::DevicestatusValue::VALUE_INVALID);
    dataParse_->DisableCount(type);
    DEV_HILOGD(SERVICE, "Exit");
    return ERR_OK;
}

ErrCode DeviceStatusSensorMock::NotifyMsdpImpl(const DevicestatusDataUtils::DevicestatusData& data)
{
    DEV_HILOGI(SERVICE, "Enter");
...
void DeviceStatusSensorMock::GetDeviceStatusData()
{
    ...
    for (int32_t n = int(DevicestatusDataUtils::DevicestatusType::TYPE_INVALID);
        n <= DevicestatusDataUtils::DevicestatusType::TYPE_LID_OPEN; ++n) {
        if (enabledType_[n] == DevicestatusDataUtils::DevicestatusValue::VALUE_INVALID) {
            DevicestatusDataUtils::DevicestatusType type = DevicestatusDataUtils::DevicestatusType(n);
...
{
    DEV_HILOGI(SERVICE, "Enter");
    g_sensorMock = new DeviceStatusSensorMock();
    return g_sensorMock;
}

extern "C" void Destroy(const DevicestatusSensorInterface* algorithm)
{
    DEV_HILOGI(SERVICE, "Enter");
    delete algorithm;
}
