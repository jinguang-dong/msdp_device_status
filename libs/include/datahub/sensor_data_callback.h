...
class SensorDataCallback final : public DelayedRefSingleton<SensorDataCallback> {
    DECLARE_DELAYED_REF_SINGLETON(SensorDataCallback);
public:
    DISALLOW_COPY_AND_MOVE(SensorDataCallback);
    bool RegisterCallbackSensor(int32_t sensorTypeId);
    bool UnregisterCallbackSensor(int32_t sensorTypeId);
    void Init();
    bool Unregister();
    bool SubscribeSensorEvent(const int32_t sensorTypeId, SensorCallback callback);
    bool UnSubscribeSensorEvent(const int32_t sensorTypeId, SensorCallback callback);
