...
{
    DEV_HILOGI(SERVICE, "MotionMsdpRdbInit: enter");
    if (!pointEventCb_) {
    ...
    DEV_HILOGI(SERVICE, "MotionMsdpRdbInit: exit");
    return true;
}
...
ErrCode MotionGestureManager::RegisterCallback(std::shared_ptr<MsdpAlgorithmCallback>& callback)
{
    ...
    || type_ == MotionDataUtils::TYPE_THREE_FINGERS_SLIDE) {
        ...
        DEV_HILOGI(SERVICE, "TYPE_POCKET: enter");
        if (!pocket_) {
            DEV_HILOGI(SERVICE, "MotionMsdpRdbInit_pocket: enter");
...
{
    DEV_HILOGI(SERVICE, "UnregisterCallback enter");
