...
bool VirtualDeviceBuilder::ExecuteUnmount(const char *id, const char *name, std::ifstream &stream,
    const std::string &direntName)
{
...
void VirtualDeviceBuilder::Unmount(const char *name, const char *id)
{
    ...
    while ((dent = readdir(procDir)) != nullptr) {
        ...
        if (!ExecuteUnmount(id, name, stream, direntName)) {
            goto EXIT;
        }
    }
