...
void DeviceStatusClientFuzzer::TestSubscribeCallback(const uint8_t* data)
{
    ...
    STATIONARY_MGR->SubscribeCallback(static_cast<Type>(type[0]), ActivityEvent::ENTER_EXIT, ReportLatencyNs::LONG, cb);
...
void DeviceStatusClientFuzzer::TestGetDevicestatusData(Type type)
{
    ...
    STATIONARY_MGR->GetDeviceStatusData(type);
...
void DeviceStatusClientFuzzer::TestUnSubscribeCallback(Type type)
{
    ...
    STATIONARY_MGR->UnsubscribeCallback(type, ActivityEvent::ENTER_EXIT, cb);
}
