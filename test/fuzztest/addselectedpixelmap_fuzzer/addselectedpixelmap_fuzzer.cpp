...
#include "addselectedpixelmap_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "AddSelectedPixelMapFuzzTest"
...
bool AddSelectedPixelMapFuzzTest(const uint8_t* data, size_t size)
{
    ...
    DelayedSingleton<DeviceStatusService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(Msdp::DeviceInterfaceCode::ADD_SELECTED_PIXELMAP), datas, reply, option);
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::AddSelectedPixelMapFuzzTest(data, size);
    return 0;
}
} // namespace DeviceStatus
} // namespace Msdp
