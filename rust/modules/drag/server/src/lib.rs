...
use fusion_data_rust::{CallingContext, DragData, IPlugin};
...
///     * `IPlugin::set_param` to update shadow, using [`UPDATE_SHADOW`] as
///       parameter ID.
///
    ...
    fn start(
        ...
        match data.read::<DragData>() {
            ...
            Err(_) => {
                error!(
                    LOG_LABEL,
                    "In FusionDragServer::start(): DragData::deserialize() failed"
                );
                Err(FusionErrorCode::Fail)
            }
        }
