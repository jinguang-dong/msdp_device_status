...
use fusion_data_rust::{AllocSocketPairParam, BasicParamID, CallingContext, IPlugin};
...
impl FusionBasicServer {
    fn alloc_socket_pair(&self, data: &mut MsgParcel, reply: &mut MsgParcel) -> FusionResult<()> {
        let call_param = data
            .read::<AllocSocketPairParam>()
            .or(Err(FusionErrorCode::Fail))?;
        ...
        if let Some(proxy) = FusionService::get_instance() {
            ...
            proxy.alloc_socket_fd(
                call_param.program_name.as_str(),
                call_param.module_type,
                &mut client_fd,
                &mut token_type,
            )?;
            ...
            reply.write_file(f).or(Err(FusionErrorCode::Fail))?;
            reply.write(&token_type).or(Err(FusionErrorCode::Fail))?;
