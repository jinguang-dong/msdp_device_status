...
/// bring net packet module.
pub mod net_packet;
mod binding;
use hilog_rust::{error, hilog, debug, HiLogLabel, LogType};
use std::ffi::{CString, c_char};
use std::mem::size_of;
use binding::CStreamServer;
use binding::CClient;
use net_packet::{NetPacket, PackHead};
...
pub struct StreamBuffer {
    /// an error occur in the reading and the writing process
    pub rw_error_status: ErrorStatus,
    /// record counts of the writing
    pub r_count: usize,
    /// record counts of the reading
    pub w_count: usize,
    /// record position of the reading
    pub r_pos: usize,
    /// record position of the writing
    pub w_pos: usize,
    /// the buffer of the writing and reading
    pub rw_buff: [c_char; MAX_STREAM_BUF_SIZE + 1],
}
...
impl Default for StreamBuffer {
    fn default() -> Self {
        Self {
            ...
            r_pos: 0,
            w_pos: 0,
            rw_buff: [0; MAX_STREAM_BUF_SIZE + 1],
        }
    }
}
...
impl StreamBuffer {
    ...
    pub(crate) fn data(&self) -> *const c_char {
        &(self.rw_buff[0]) as *const c_char
    }
    ...
    fn read_buf(&self) -> *const c_char {
        &(self.rw_buff[self.r_pos]) as *const c_char
    }
    fn write_char_usize(&mut self, buf: *const c_char, size: usize) -> bool {
        ...
        let pointer = &(self.rw_buff[0]) as *const c_char;
    ...
    fn copy_data_to_begin(&mut self) {
        ...
        if unread_size > 0 && self.r_pos > 0 {
            for (index, value) in (self.r_pos..=self.w_pos).enumerate() {
                self.rw_buff[index] = self.rw_buff[value];
            }
        }
    ...
    /// circle write stream buffer
    ///
    /// # Safety
    ///
    /// It is used to write stream buffer circled.
    ...
    /// Set callback function of server
    ///
    /// # Safety
    ///
