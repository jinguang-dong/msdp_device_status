...
extern crate devicestatus_rust_socket;
extern crate libc;
use libc::c_int;
use libc::c_char;
use devicestatus_rust_socket::stream_session::StreamSession;
use devicestatus_rust_socket::stream_buffer::net_packet::NetPacket;
use devicestatus_rust_socket::stream_buffer::net_packet::MessageId;
use devicestatus_rust_socket::epoll_manager::EpollManager;
use devicestatus_rust_socket::stream_buffer::{StreamBuffer, MAX_PACKET_BUF_SIZE};
use std::mem::size_of;
const TEST1: i32 = 777;
const TEST2: i32 = 888;
const TEST3: i32 = 999;

fn read_packets_callback(pkt: *mut NetPacket) {
...
fn main() {
    ...
    /// Call libc function socketpair for creating socket pairs.
    unsafe {
    ...
    unsafe {
        ...
        if pid == -1 {
            ...
            let mut stream_socket: EpollManager = Default::default();
            ...
            if EpollManager::epoll_ctl(client_fd, libc::EPOLL_CTL_ADD, &mut ev, stream_socket.epoll_fd) != 0 {
                println!("epoll_ctl fail");
            }
            loop {
                let _count: i32 = EpollManager::epoll_wait(&mut events[0], MAX_EVENT_SIZE, -1, stream_socket.epoll_fd);
