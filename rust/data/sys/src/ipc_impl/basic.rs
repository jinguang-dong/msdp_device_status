...
use fusion_utils_rust::{call_debug_enter, define_enum, FusionErrorCode, FusionResult};
...
impl AllocSocketPairParam {
    ...
    pub unsafe fn from_c(program_name: *const c_char, module_type: i32) -> FusionResult<Self> {
        ...
        let cs = unsafe { CStr::from_ptr(program_name) };
        match cs.to_str() {
            Ok(sref) => Ok(Self {
            ...
            Err(_) => {
                error!(
                    LOG_LABEL,
                    "Can not convert \'program_name\' from CStr to String"
                );
