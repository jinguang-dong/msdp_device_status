...
use fusion_utils_rust::{call_debug_enter, err_log, FusionErrorCode, FusionResult};
use hilog_rust::{error, hilog, HiLogLabel, LogType};

use crate::binding::{
    CIProfileEventCb, CIProfileEvents, CISubscribeInfos, CProfileChangeNotification,
    CSubscribeInfo, SubscribeProfileEvents, UnsubscribeProfileEvents, RET_OK,
};
...
impl TryFrom<u32> for ProfileEvent {
    ...
    fn try_from(value: u32) -> Result<Self, Self::Error> {
        match value {
            ...
            _ if u32::from(ProfileEvent::ProfileChanged) == value => {
                Ok(ProfileEvent::ProfileChanged)
            }
...
/// Represents a notification for profile changes, containing affected profile
/// entries, device ID, and whether the change is local.
...
impl ProfileEventCallback {
    /// Structures with C-compatible layouts that safely interconvert the first
    /// structure field and structure instance. Based on the C17 standard
    ///
    /// # Note
    ///
    /// This function performs a conversion of the pointer type to `*mut Self`
    /// and returns it. Please note that the pointer `cb` is a raw pointer
    /// that needs to be handled carefully to avoid memory safety issues and
    /// undefined behavior. Make sure that the returned pointer is not null
    ...
    fn from_interface(cb: *mut CIProfileEventCb) -> Option<&'static mut Self> {
        ...
        unsafe { cb_ptr.as_mut() }
    }
    /// Clone a `CIProfileEventCb` instance.
    ///
    /// # Note
    ///
    /// Please note that the pointer `cb` is a raw pointer that needs to be
    /// handled carefully to avoid memory safety issues and undefined
    /// behavior. Make sure to properly dereference and manipulate the data
    /// using appropriate safe Rust code.
    ...
    /// This callback function is invoked when a profile change notification
    /// occurs.
    extern "C" fn on_profile_changed(
        _cb: *mut CIProfileEventCb,
...
impl DeviceProfile {
    ...
    /// Updates the device profile with the specified
    ...
    pub fn get_device_profile(
        _udid: &str,
        _service_id: &str,
        _profile: &ServiceCharacteristicProfile,
    ) -> FusionResult<()> {
        todo!()
    }
    ...
    /// # Arguments
    ///
    /// * `subscribe_infos` - A slice of `SubscribeInfo` structures representing
    ///   the events to subscribe to.
    /// * `event_callback` - A reference to the `IProfileEventCallback` trait
    ///   object for handling profile events.
    /// * `failed_events` - A mutable vector to store any events that failed to
    ///   subscribe.
    ///
    /// # Returns
    ///
    /// An empty `FusionResult` indicating success or an error if subscribing to
    /// events fails.
    pub fn subscribe_profile_events(
        &self,
        subscribe_infos: &[SubscribeInfo],
        event_callback: &Arc<dyn IProfileEventCallback>,
        failed_events: &mut Vec<ProfileEvent>,
    ) -> FusionResult<()> {
        ...
        // SAFETY: no `None` here, cause `subscriptions_borrowed`, `event_cb_ptr` and
        // `failed_ptr` are valid.
        let ret = unsafe {
            SubscribeProfileEvents(
                &subscriptions_borrowed,
                event_cb_ptr as *mut CIProfileEventCb,
                &mut failed_ptr,
            )
        };
        if !failed_ptr.is_null() {
            ...
            let profile_events_slice = unsafe {
                std::slice::from_raw_parts(
                    (*failed_ptr).profile_events,
                    (*failed_ptr).num_of_profile_events,
                )
            };
    ...
    /// # Arguments
    ///
    /// * `profile_events` - A slice of `ProfileEvent` enumerations representing
    ...
    /// # Returns
    ///
    /// An empty `FusionResult` indicating success or an error if unsubscribing
    /// from events fails.
    pub fn unsubscribe_profile_events(
        &self,
        ...
        // SAFETY:  no `None` here, cause `profile_events_borrowed`, `event_cb_ptr` and
        // `failed_ptr` are valid.
        let ret = unsafe {
            UnsubscribeProfileEvents(
                &profile_events_borrowed,
                event_cb_ptr as *mut CIProfileEventCb,
                &mut failed_ptr,
            )
        };
    ...
    pub fn sync_device_profile(
        _sync_options: &SyncOptions,
        _sync_callback: &Arc<dyn IProfileEventCallback>,
    ) -> FusionResult<()> {
        todo!()
    }
}
